...
#### initializing parameters
	...
	MaxNumberOfClosebyStations = 5
	DeltaTimeInMinutes = 10 # for defining neighborhood around time of arrival
	...
	for i in range(len(ClosestStations)):
	    ResultsTable.append(dict(stationName=locations[ClosestStations.ind[i],0],distance=ClosestStations.StartToEndDistance[i],traveltime=0, probNoAvailDocks=0, stationID =  int(ClosestStations.id[i])    )    )
    ...
    for i in range(MaxNumberOfClosebyStations):
        print ([i,len(ResultsTable)])
        endstationID = int(ResultsTable[i+MaxNumberOfClosebyStations]['stationID'])
        startstationID = int(ResultsTable[i]['stationID'])
        indStartStationID = pd.Index(d1.id).get_loc(str(startstationID))
        ...
        StartTime = datetime.datetime(2014,StartMonth,StartDay,StartHour,StartMinute,00)
        ...
        if StartTimeHigh.hour < StartTimeLow.hour:
            TimeSelectString =  " and ((TIME(starttime) between '00:00:00' and '" + str(StartTimeHigh.time()) + "') or (TIME(starttime) between '" + str(StartTimeLow.time()) + "' and '23:59:59')  ) "
        else:
            TimeSelectString =  " and TIME(starttime) between '" + str(StartTimeLow.time()) + "' and '" + str(StartTimeHigh.time()) + "' "

        if StartDay <=5:
            DaySelectString  = " and DAYOFWEEK(starttime)<=5 " # weekend or not
        else:
            DaySelectString  = " and DAYOFWEEK(starttime)>5 "

        if StartMonth == 6:
            MonthSelectString  = " and startmonth = 5" + " " # don't have data for this month :)
        else:
            MonthSelectString  = " and startmonth  = "+str(StartMonth) + " "

        sql1= "select starthour + startminutes/60.0 as time,tripduration from CityBikes.TripData where startstationid="\
                +str(startstationID) +' and endstationid = '+ str(endstationID) + TimeSelectString + DaySelectString + MonthSelectString
        rr1 = pd.read_sql(sql1, db)
        if len(rr1) <2:
            MonthSelectString = '';DaySelectString=''
            sql1= "select starthour + startminutes/60.0 as time,tripduration from CityBikes.TripData where startstationid="\
                    +str(startstationID) +' and endstationid = '+ str(endstationID) + TimeSelectString + DaySelectString + MonthSelectString
            rr1 = pd.read_sql(sql1, db)
            if len(rr1) < 2:
        ...
        if len(rr1)>=2:
            #plt.scatter(rr1.time,rr1.tripduration)
            #plt.show()
            slope, intercept, r_value, p_value, std_err = stats.linregress(rr1.time,rr1.tripduration)
            ExpectedTravelTime = int(slope*(StartHour+StartMinute/60.0)+ intercept);
            ExpectedTravelTimeString = str((datetime.timedelta(seconds=ExpectedTravelTime)));ExpectedTravelTimeString = np.percentile(rr1.tripduration,50)
            ResultsTable[i+MaxNumberOfClosebyStations]['traveltime']=ExpectedTravelTimeString
        else:
            ExpectedTravelTimeString = 'no data'
            ResultsTable[i+MaxNumberOfClosebyStations]['traveltime']='no data'

##############################################################################
######  calculating the probabilities for the end stations
        ...
        if EndTimeHigh.hour < EndTimeLow.hour:
            TimeSelectString =  "  ((TIME(time) between '00:00:00' and '" + str(EndTimeHigh.time()) + "') or (TIME(time) between '" + str(EndTimeLow.time()) + "' and '23:59:59')  ) "
        else:
            TimeSelectString =  "  TIME(time) between '" + str(EndTimeLow.time()) + "' and '" + str(EndTimeHigh.time()) + "' "

        if EndDay <=5:
            DaySelectString  = " DAYOFWEEK(time)<=5 "
        else:
            DaySelectString  = " DAYOFWEEK(time) >5 "

        if EndMonth == 6:
            MonthSelectString  = " month = 7 "
        else:
            MonthSelectString  = " month  = "+str(EndMonth) + " "

        sql2 = "select count(AvailableBikes) as NumDataPoints, sum(case when AvailableBikes<="+	str(Tolerance) +" then 1 else 0 end)/count(AvailableBikes) as probNoAvailBikes from CityBikes.station"+str(startstationID)+"  where "+ TimeSelectString + " and " + DaySelectString + " and " + MonthSelectString
        probNoAvailBikes = pd.read_sql(sql2, db).probNoAvailBikes[0]
        print(probNoAvailBikes)
        ResultsTable[i]['probNoAvailDocks'] = probNoAvailBikes
...
######  calculating the probabilities for the end stations
        ...
        sql11 = "select max(AvailableBikes) as MaxAvailableBikes from CityBikes.station"+str(endstationID)+"  where " + MonthSelectString
        ...
        sql3 = "select count(AvailableBikes) as NumDataPoints, sum(case when AvailableBikes>="+str(TotalDockCapacity-Tolerance) +" then 1 else 0 end)/count(AvailableBikes) as probNoAvailDocks from CityBikes.station"+str(endstationID)+"  where "+ TimeSelectString + " and " + DaySelectString + " and " + MonthSelectString
        probNoAvailDocks = pd.read_sql(sql3, db).probNoAvailDocks[0]
        print(probNoAvailDocks)
        ResultsTable[i+MaxNumberOfClosebyStations]['probNoAvailDocks'] = probNoAvailDocks
...
#### initializing parameters
	...
	ClosestStations = ClosestStations.reset_index()
	for i in range(len(ClosestStations)):
            EndStationsResultsTable = EndStationsResultsTable.append(dict(stationName=locations[ClosestStations.ind[i],0],distance=ClosestStations.StartToEndDistance[i],traveltime=0, traveltimemin = 0, traveltimemax=0, prob=-1, probstring="sorry, no data!", stationID =  int(ClosestStations.id[i]),isprefered= "", score=0    ) , ignore_index=True   )
...
##########################################################################################################################################
	...
	for i in range(MaxNumberOfClosebyStations):
		...
		probNoAvailBikes = pd.read_sql(sql2, db).probNoAvailBikes[0]
		try:
			if len(probNoAvailBikes)<1:
				sql2 = "select count(AvailableBikes) as NumDataPoints, sum(case when AvailableBikes<="+	str(Tolerance) +" then 1 else 0 end)/count(AvailableBikes) as probNoAvailBikes from CityBikes.station"+str(startstationID)+"  where " + TimeSelectString + ' and '+ DaySelectString
				probNoAvailBikes = pd.read_sql(sql2, db).probNoAvailBikes[0]
		...
		sql2 = "select count(AvailableBikes) as NumDataPoints, sum(case when AvailableBikes>="+str(TotalDockCapacity-Tolerance) +" then 1 else 0 end)/count(AvailableBikes) as probNoAvailDocks from CityBikes.station"+str(endstationID)+"  where "+ TimeSelectString + " and " + DaySelectString + " and " + DateSelectString
		probNoAvailDocks = pd.read_sql(sql2, db).probNoAvailDocks[0]
		try:
			if len(probNoAvailDocks)<1:
				sql2 = "select count(AvailableBikes) as NumDataPoints, sum(case when AvailableBikes>="+str(TotalDockCapacity-Tolerance) +" then 1 else 0 end)/count(AvailableBikes) as probNoAvailDocks from CityBikes.station"+str(endstationID)+"  where "+ TimeSelectString + " and " + DaySelectString
				probNoAvailDocks = pd.read_sql(sql2, db).probNoAvailDocks[0]
...
## old
	...
	DeltaTimeInSeconds = DeltaTimeInMinutes*60
    DeltaDays =15;
	StartStationsResultsTable = pd.DataFrame(columns=('stationName','distance','traveltime','prob','stationID','isprefered'))
	EndStationsResultsTable = pd.DataFrame(columns=('stationName','distance','traveltime','prob','stationID','isprefered'))
    ## Finding the closest stations to the chosen start station
	DistanceList = numpy.zeros((len(d1), 4 ))
	for i in range((len(d1))):
	    DistanceList[i][0] = distanceLatLon(float(d1.latitude[i]), float(d1.longitude[i]), float(d1.latitude[indStartStationID]), float(d1.longitude[indStartStationID]))
	...
	ClosestStations = ClosestStations.reset_index()
	for i in range(len(ClosestStations)):
        StartStationsResultsTable = StartStationsResultsTable.append(dict(stationName=locations[ClosestStations.ind[i],0],distance=ClosestStations.StartToEndDistance[i],traveltime=0, prob=0, stationID =  int(ClosestStations.id[i]),isprefered= ""    ) , ignore_index=True   )
	...
	ClosestStations = ClosestStations.reset_index()
	for i in range(len(ClosestStations)):
        EndStationsResultsTable = EndStationsResultsTable.append(dict(stationName=locations[ClosestStations.ind[i],0],distance=ClosestStations.StartToEndDistance[i],traveltime=0, prob="less than 5", stationID =  int(ClosestStations.id[i]),isprefered= ""    ) , ignore_index=True   )
    ...
    for i in range(MaxNumberOfClosebyStations):
        ...
        DateSelectString = " (starttime between DATE('"+ str(StartDateLow) + "') and DATE('" + str(StartDateHigh) + "')) "
        sql1= "select tripduration from CityBikes.TripData where startstationid="\
                +str(startstationID) +' and endstationid = '+  str(endstationID) + ' and '+ TimeSelectString + ' and '+ DaySelectString + ' and '+ DateSelectString
        rr1 = pd.read_sql(sql1, db)
        if len(rr1) <1: # if no trips found, relax constraints
            sql1= "select tripduration from CityBikes.TripData where startstationid="\
                    +str(startstationID) +' and endstationid = '+  str(endstationID) + ' and '+ DaySelectString + ' and '+ DateSelectString
            rr1 = pd.read_sql(sql1, db)
            if len(rr1) < 1: # if no trips found, relax constraints
                sql1= "select tripduration from CityBikes.TripData where startstationid="\
                    +str(startstationID) +' and endstationid = '+  str(endstationID) + ' and ' + DateSelectString
                rr1 = pd.read_sql(sql1, db)
        ...
        if len(rr1)>=1:
            ...
            ExpectedTravelTime=0;ExpectedTravelTimeString = 'nobody has done this trip!'

        print(ExpectedTravelTimeString)
        EndStationsResultsTable.traveltime[i] = ExpectedTravelTimeString
        ##############################################################################
        ######  calculating the probability of no bikes available for the start stations
        if StartTimeHigh.hour < StartTimeLow.hour:
            TimeSelectString =  "  ((TIME(time) between '00:00:00' and '" + str(StartTimeHigh.time()) + "') or (TIME(time) between '" + str(StartTimeLow.time()) + "' and '23:59:59')  ) "
    ...
    ResultsTable=[]
	for i in range(len(StartStationsResultsTable)):
	    ResultsTable.append(dict(stationName=StartStationsResultsTable.stationName[i],distance=StartStationsResultsTable.distance[i],traveltime=StartStationsResultsTable.traveltime[i], prob=StartStationsResultsTable.prob[i], stationID = StartStationsResultsTable.stationID[i],isprefered=StartStationsResultsTable.isprefered[i] ) )

	for i in range(len(EndStationsResultsTable)):
	    ResultsTable.append(dict(stationName=EndStationsResultsTable.stationName[i],distance=EndStationsResultsTable.distance[i],traveltime=EndStationsResultsTable.traveltime[i], prob=EndStationsResultsTable.prob[i], stationID = EndStationsResultsTable.stationID[i],isprefered=EndStationsResultsTable.isprefered[i] ) )
...
############################################################################################################################################################
####  using now linear regression or logistic regression

# estimating the travel time:

sql1= "select tripduration,startmonth,startday,starthour from CityBikes.TripData where startstationid="\
        +str(startstationID) +' and endstationid = '+ str(endstationID)
rr1 = pd.read_sql(sql1, db)
    ...
    if len(rr1>=2):
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(rr1.time,rr1.tripduration)
        ExpectedTravelTime = int(slope*(StartHour+StartMinute/60.0)+ intercept)
        ExpectedTravelTimeString = str((datetime.timedelta(seconds=ExpectedTravelTime)))
        ResultsTable[i]['traveltime']=ExpectedTravelTimeString
    else:
        ExpectedTravelTimeString = 'no data'
        ResultsTable[i]['traveltime']='no data'
...
sql2 = "select time,WEEKDAY(time) as dayweek ,hour,"+ str(EndStationTotalDocks) + "-AvailableBikes as NumOpenDocks from CityBikes.station"+str(endstationID) + "  "  #  +"  where "+ TimeSelectString + DaySelectString + MonthSelectString
rr2 = pd.read_sql(sql2, db)
...
ass sklearn.linear_model.LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, class_weight=None, random_state=None)¶
...
plt.scatter(rr2.hour,rr2.AvailableBikes)
plt.show()
...
"select hour, minute,sum(case when AvailableBikes="+  0 then 1 else 0 end)/count(AvailableBikes), sum(AvailableBikes) as AvailableBikes,count(*) as counts
from CityBikes.station477 WHERE TIME(time) between '20:00:00' and '21:00:00' group by minute"
